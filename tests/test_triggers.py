...
@pytest.mark.db
def test_the_drift_reference_is_built_once_and_read_back(fresh_db: None, tmp_path: Any) -> None:
    """It is a fixed function of the champion, and re-reading the training window
    every day was most of what triggers pulled out of the database. The second
    call must not need the rows at all — they are deleted in between."""
            ...
            conn.execute(
                ...
                INSERT INTO landing.rc_events
                    (revid, event_ts, ns, title, user_name, user_id, is_anon, is_temp,
                     is_minor, is_bot, oldlen, newlen, tags, sampling_stratum,
                     sampling_weight, ingested_at_utc)
                VALUES (%s, %s, 0, %s, %s, 500, false, false, false, false,
                        100, %s, '{}', 'registered', 33.3, now())
                """,
                (i + 1, start + timedelta(minutes=i), f"Page{i % 7}", f"User{i % 5}", 100 + i),
            )
    champion = {
        "model_version": "champ",
    ...
    with connect() as conn:
        built = triggers._reference_columns(conn, champion, str(tmp_path))
    assert built is not None and (tmp_path / "champ.npz").exists()

    with connect() as conn:
        conn.execute("TRUNCATE landing.rc_events CASCADE")
        assert triggers._reference_columns(conn, champion, None) is None, "the rows are gone"
        kept = triggers._reference_columns(conn, champion, str(tmp_path))

    assert kept is not None
    for name in triggers.MONITORED_FEATURES:
        assert np.array_equal(kept[name], built[name]), name
