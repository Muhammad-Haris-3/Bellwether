...
// When the numbers were read, as a fixed instant rather than "4 minutes ago".
//
// A relative phrase would be computed once, at render, and then frozen into
// HTML that is served for the next five minutes — longer if the API is down,
// because a refresh that fails leaves the previous copy in place. "Moments ago"
// would be a lie by the time most people read it, and it is precisely the lie
// this project exists to refuse. An absolute instant stays true however long
// the copy is served, and it cannot disagree between server and browser.
function readAt(fetchedAt: string): string {
  const t = new Date(fetchedAt);
  if (Number.isNaN(t.getTime())) return "an unknown time";
...
// The figures themselves. Takes a snapshot rather than fetching one, so the
// same markup serves the cached server render and the browser fallback.
export function StatusBody({ snapshot }: { snapshot: StatusSnapshot }) {
  const { stats, kc2, metrics } = snapshot;
  const totals = stats.totals;
  const overallRate =
    totals.events > 0 ? (totals.tagged_at_ingest / totals.events) * 100 : null;
    ...
    <>
      <ScrollReveal>
        ...
        <div className="grid">
          ...
          <div className="card">
            <div className="value">{totals.revert_events.toLocaleString()}</div>
        ...
        <p className="note">
          {overallRate !== null && (
            <>
              ...
              revert tag when we first saw them. A floor, not a rate — the tag
              array is frozen at ingestion, so for a live edit it cannot yet
              know about a revert that has not happened.
            </>
          )}
        </p>
      </ScrollReveal>
      ...
      <ScrollReveal>
            ...
            <tbody>
              {stats.mature_48h.map((row) => (
                <tr key={row.stratum}>
                  ...
                  <td className="muted">{row.weighted_n.toLocaleString()}</td>
                  <td>
        ...
        <p className="note">
          Logged-out edits are sampled at 50% and registered ones at 3%, so the
          sample is not the population. Both rates are shown — publishing only
          one would mean choosing which.
        </p>
      </ScrollReveal>
      ...
      */}
      <ScrollReveal>
        ...
        <p className="note">
          ...
          harvested at leisure, with folds trained on data adjacent to what they
          score. The live rows grade forecasts written to an append-only
        ...
        <p className="note">
          The two live rows are also not interchangeable. The maturity cohort is
          the tenth of events the labeller checks densely enough to grade at 48
          hours; everything else waits for its seven-day check. The cohort
          figure arrives first over far fewer events.
        </p>
        {metrics?.populations?.all?.windows?.["all"] && (
          <p className="note">
            Excluded from the live figures:{" "}
            {metrics.populations.all.windows[
              "all"
            ].excluded.immature.toLocaleString()}{" "}
            not yet matured, and{" "}
            {metrics.populations.all.windows[
              "all"
            ].excluded.late.toLocaleString()}{" "}
            scored after their own outcome was already visible. The second
            exclusion is correct and is not neutral — those edits were reverted
            fast, so dropping them removes real positives, which is why their
            own base rate is published beside the count.
          </p>
        )}
      </ScrollReveal>
      ...
      <ScrollReveal>
        ...
        {(() => {
          ...
          if (paired?.margin != null) {
            return (
              <p>
                Bellwether {pct(paired.model_pr_auc_on_paired)} against Lift Wing{" "}
                {pct(paired.liftwing_pr_auc)} on {paired.n} events both models
                scored — a margin of {pct(paired.margin)}.
              </p>
            );
          }
          return (
        ...
        <p className="note">
          ...
          whichever way it falls. It is paired on the events both models scored,
          which is why the Bellwether figure quoted here is not the one in the
          table above.
        </p>
      </ScrollReveal>
      ...
      <ScrollReveal>
        ...
        <p className="note">
          Staleness is stated rather than implied. Counts with no indication of
          when they last changed look identical to counts that stopped changing
          days ago.
        </p>
        <p className="note">
          These figures were read from the API at {readAt(snapshot.fetchedAt)}{" "}
          and are served from a cache that refreshes in the background. The API
          sleeps when idle; caching is what stops that from becoming your wait.
        </p>
      </ScrollReveal>
    </>
  );
